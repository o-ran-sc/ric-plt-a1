...
def temp_rmr_rcvall_msgs(mrc, pass_filter=[]):
    """
    this is temporary and will be removed as soon as a bugfix is made in rmr python
    """
                ...
                if mtype == A1_POLICY_RESPONSE:
                    ...
                    try:
                        ...
                        for pii in data.get_instance_list(pti):
                            ...
                            # use rts to send it back
                            for _ in range(0, RETRY_TIMES):
                                rmr.set_payload_and_length(payload, sbuf)
                                sbuf.contents.mtype = A1_POLICY_REQUEST
                                pre_reply_summary = rmr.message_summary(sbuf)
                                sbuf = rmr.rmr_rts_msg(self.mrc, sbuf)
                                post_reply_summary = rmr.message_summary(sbuf)

                                if post_reply_summary["message status"] == "RMR_OK":
                                    break
                                mdc_logger.debug("Message NOT sent!")
                                mdc_logger.debug(
                                    "Pre-send: {0}, Post-send: {1}".format(pre_reply_summary, post_reply_summary)
                                )
                    except (PolicyTypeNotFound, PolicyInstanceNotFound):
