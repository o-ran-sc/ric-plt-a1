...
class _RmrLoop:
    ...
    def loop(self):
        ...
        while self.keep_going:
            ...
            while not self.work_queue.empty():
                ...
                self.send(payload=work_item["payload"], message_type=work_item["msg type"])
    ...
    def send(self, payload, message_type=0):
        """
        Tries to sends a message up to RETRY_TIMES
        """
        ...
        for _ in range(0, RETRY_TIMES):
            ...
            logger.debug("Pre message send summary: %s", rmr.message_summary(sbuf))

            # send
            sbuf = rmr.rmr_send_msg(self.mrc, sbuf)
            post_send_summary = rmr.message_summary(sbuf)
            logger.debug("Post message send summary: %s", post_send_summary)
