...
def delete_policy_instance(policy_type_id, policy_instance_id):
    ...
    if vector == []:
        ...
        clos = lambda: _delete_after(policy_type_id, policy_instance_id, INSTANCE_CREATE_ACK_TIMEOUT)
    else:
        # handler is not empty, we wait max t1,t2 to expire then goodnight
        clos = lambda: _delete_after(
            policy_type_id, policy_instance_id, max(INSTANCE_DELETE_ACK_TIMEOUT, INSTANCE_CREATE_ACK_TIMEOUT)
        )
