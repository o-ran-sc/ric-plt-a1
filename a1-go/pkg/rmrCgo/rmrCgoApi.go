...
func (*Context) Init(port string, maxMsgSize int, flags int) RmrMessenger {
	pp := C.CString(port)
	defer C.free(unsafe.Pointer(pp))
	// logger.Debugf("#rmrCgoApi.Init - Going to initiate RMR router")
	ctx := NewContext(maxMsgSize, flags, C.rmr_init(pp, C.int(maxMsgSize), C.int(flags)))
	start := time.Now()
...
func (ctx *Context) SendMsg(msg *MBuf, printLogs bool) (*MBuf, error) {
	...
	allocatedCMBuf := ctx.getAllocatedCRmrMBuf( msg, ctx.MaxMsgSize)
